...
: Synchronizer<R>(N,N), pilot_size(pilot_values.size()), pilot_values(pilot_values), pilot_start(pilot_start), pilot_nbr(pilot_start.size()), R_l(2,(R)0.0)
{
	assert(pilots_sz > 0);
	assert(pilot_size > 0);
...
::_synchronize(const R *X_N1, R *Y_N2, const int frame_id)
{
	...
	for  (int p = 0 ; p<P ; p++)
	{
		...
		for (int i = 0 ; i<Lp ; i++)
		{
			z[2*i]     = X_N1[2*i +   + this->pilot_start[p]] * pilots[2*i] 
			           + X_N1[2*i + 1 + this->pilot_start[p]] * pilots[2*i + 1];

			z[2*i + 1] = X_N1[2*i + 1 + this->pilot_start[p]] * pilots[2*i] 
			           - X_N1[2*i     + this->pilot_start[p]] * pilots[2*i + 1];
		}
	...
	{
		float t = (R)(2 * n) / (R)this->N;
		Y_N2[2*n]   = X_N1[2*n]     * cos( 2*M_PI * est_reduced_freq * t) 
		            + X_N1[2*n + 1] * sin( 2*M_PI * est_reduced_freq * t);

		Y_N2[2*n+1] = X_N1[2*n + 1] * cos( 2*M_PI * est_reduced_freq * t)
		            - X_N1[2*n]     * sin( 2*M_PI * est_reduced_freq * t);
	}
}
