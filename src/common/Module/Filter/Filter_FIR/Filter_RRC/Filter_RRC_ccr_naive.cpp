...
::Filter_RRC_ccr_naive(const int N, const R rolloff, const int samples_per_symbol, const int delay_in_symbol,
                       const int n_frames)
: Filter_FIR_ccr_naive<R>(N, this->compute_rrc_coefs(rolloff, samples_per_symbol, delay_in_symbol), n_frames),
                          rolloff(rolloff), samples_per_symbol(samples_per_symbol), delay_in_symbol(delay_in_symbol)
{
}
