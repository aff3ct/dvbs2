...
::Filter_FIR_ccr_naive(const int N, const std::vector<R> b, const int n_frames)
: Filter<R>(N, N, n_frames), b(b.size(), R(0)), buff(2*b.size(), std::complex<R>(R(0))), head(0), size((int)b.size())
{
	assert(size > 0);
