...
::get_description(cli::Argument_map_info &args) const
{
	...
	args.add({"sim-inter-fra","F"},  cli::Integer(cli::Positive(), cli::Non_zero()),               "Inter frame level."                                              );
...
::store(const cli::Argument_map_value &vals)
{
	...
	n_frames                 = vals.exist({"sim-inter-fra","F"}  ) ? vals.to_int  ({"sim-inter-fra","F"} ) : 1           ;
	...
	src_no_loop              = vals.exist({"src-no-loop"}        ) ? true                                  : false       ;
	...
	p_rad.N = (this->pl_frame_size) * p_shp.osf;
	p_rad.n_frames = n_frames;
	p_rad.store(vals);

	p_sff.N        = this->pl_frame_size;
	p_sff.n_frames = n_frames;
	p_sff.store(vals);

	p_sfc.N        = this->pl_frame_size*p_shp.osf;
	p_sfc.n_frames = n_frames;
	p_sfc.osf      = p_shp.osf;
	p_sfc.ref_freq_shift = max_freq_shift;
	p_sfc.store(vals);

	p_stm.N         = p_shp.osf*this->pl_frame_size;
	p_stm.osf       = p_shp.osf;
	p_stm.n_frames  = n_frames;
	p_stm.ref_delay = max_delay;
	p_stm.store(vals);

	p_sfm.N         = this->pl_frame_size;
	p_sfm.n_frames  = n_frames;
	p_sfm.ref_delay = 2 * this->p_shp.grp_delay + ((int)std::floor(this->max_delay) + 1)/this->p_shp.osf;
	p_sfm.store(vals);
...
::get_names() const
{
	...
	n.push_back(this->p_sfc.get_name());
	n.push_back(this->p_stm.get_name());
	n.push_back(this->p_sfm.get_name());
...
::get_short_names() const
{
	...
	sn.push_back(this->p_sfc.get_short_name());
	sn.push_back(this->p_stm.get_short_name());
	sn.push_back(this->p_sfm.get_short_name());
...
::get_prefixes() const
{
	...
	p.push_back(this->p_sfc.get_prefix());
	p.push_back(this->p_stm.get_prefix());
	p.push_back(this->p_sfm.get_prefix());
...
::get_headers(std::map<std::string,tools::header_list>& headers, const bool full) const
{
	...
	this->p_sfm.get_headers(headers);
...
{
	module::Encoder_BCH<B>* enc = nullptr;

	if (params.n_frames == 1)
		enc = new module::Encoder_BCH_DVBS2      <B>(params.K_bch, params.N_bch, poly_gen);
	else
		enc = new module::Encoder_BCH_inter_DVBS2<B>(params.K_bch, params.N_bch, poly_gen);

	enc->set_n_frames(params.n_frames);
	return enc;
}
...
{
	module::Decoder_BCH_std<B,Q>* dec = nullptr;

	dec = new module::Decoder_BCH_DVBS2<B,Q>(params.K_bch, params.N_bch, poly_gen);

	dec->set_n_frames(params.n_frames);
	return dec;
}
...
::build_ldpc_cdc(const DVBS2& params)
{
	...
	tools::Codec_LDPC<B,Q>* cdc = nullptr;

	// build ldpc codec
	cdc = p_cdc.build();

	cdc->set_n_frames(params.n_frames);
	return cdc;
}
...
{
	tools::Interleaver_core<D>* itl = nullptr;

	if (params.modcod == "QPSK-S_8/9" || params.modcod == "QPSK-S_3/5")
		itl = new tools::Interleaver_core_NO<D>(params.N_ldpc);
	else // "8PSK-S_8/9" "8PSK-S_3/5" "16APSK-S_8/9"
		itl = new tools::Interleaver_core_column_row<D>(params.N_ldpc, params.itl_n_cols, params.read_order);

	itl->set_n_frames(params.n_frames);
	return itl;
}
...
{
	module::Interleaver<D,T>* itl = nullptr;

	itl = new module::Interleaver<D,T>(itl_core);
...
{
	module::Modem_generic<B,R,Q,MAX>* mdm = nullptr;
	// mdm = new module::Modem_generic<B,R,Q,MAX>(params.N_ldpc, *cstl, false);
	mdm = new module::Modem_generic_fast<B,R,Q,MAX,MAXI>(params.N_ldpc, *cstl, false);

	mdm->set_n_frames(params.n_frames);
	return mdm;
}
...
::build_monitor(const DVBS2& params)
{
	...
	module::Monitor_BFER<B>* mnt = nullptr;

	mnt = new module::Monitor_BFER<B>(params.K_bch, params.max_fe, 0, false);

	mnt->set_n_frames(params.n_frames);
	return mnt;
}
...
::build_channel(const DVBS2& params, tools::Gaussian_noise_generator<R>& gen, const bool filtered)
{
	...
	if (params.channel_type == "AWGN")
		...
		if (filtered) chn = new module::Channel_user_add<R>(2 * params.pl_frame_size * params.p_shp.osf, params.channel_path, true);
		else          chn = new module::Channel_user_add<R>(2 * params.pl_frame_size                   , params.channel_path, true);
	else if (params.channel_type == "SYNCHRO")
		if (filtered) chn = new module::Channel_AWGN_LLR<R>(2 * params.pl_frame_size * params.p_shp.osf, gen, false);
		else          chn = new module::Channel_AWGN_LLR<R>(2 * params.pl_frame_size                   , gen, false);
	else
...
{
	return new module::Multiplier_sine_ccc_naive<R>(2*params.pl_frame_size * params.p_shp.osf, params.max_freq_shift, 1.0, params.n_frames);
}
