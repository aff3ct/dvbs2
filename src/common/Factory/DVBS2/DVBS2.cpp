...
::build_channel(const DVBS2& params, tools::Gaussian_noise_generator<R>& gen, const bool filtered)
{
	if (params.channel_type == "AWGN")
		if (filtered) return new module::Channel_AWGN_LLR<R>(2 * params.pl_frame_size * params.p_shp.osf, gen, false, params.n_frames);
		else          return new module::Channel_AWGN_LLR<R>(2 * params.pl_frame_size                   , gen, false, params.n_frames);
	else if(params.channel_type == "USER_ADD")
		if (filtered) return new module::Channel_user_add<R>(2 * params.pl_frame_size * params.p_shp.osf, params.channel_path, true, params.n_frames);
		else          return new module::Channel_user_add<R>(2 * params.pl_frame_size                   , params.channel_path, true, params.n_frames);
	else
